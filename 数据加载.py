    ...
    def __init__(self, params):
        ...
        if params.mode == 0:
            ...
            self.dev_data_loader = torch.utils.data.DataLoader(dataset_dev,
                                                               batch_size=params.batch_size,
                                                               collate_fn=dataset_dev.collate, shuffle=False)
        ...
        with open(filename, "r") as f:
            ...
            for idx, (tag, sentences_idx) in enumerate(sentences_idx_list):
                if tag in allowed_tags:
                    if self.ntags == 2:
                        tag = tag - 1 if tag == 1 else tag - 3  # 调整标签为 {0: Satire, 1: Trusted}
                    else:
...
class DataSet(torch.utils.data.TensorDataset):
    def __init__(self, data, adj, fea, params, entity_description):
        ...
        for i, a in enumerate(self.adjs):
            assert a[0].shape[0] == len(
                self.sents[i]), "dim of adj does not match the num of sent, where the idx is {}".format(i)
            assert a[4].shape[0] == len(
                self.feas[i][1]), "dim of adj does not match the num of entity, where the idx is {}".format(i)
            assert a[7].shape[0] == len(
                self.feas[i][2]), "dim of adj does not match the num of topic, where the idx is {}".format(i)
